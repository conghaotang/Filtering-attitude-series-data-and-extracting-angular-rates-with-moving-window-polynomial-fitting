...
C1b = xlsread('D:\FIR\D1BD.xls'); %C1BD
num = C1b(2400:7800,[1:2,5:7,4]); %将四元数 矢量 标量800:4400 2000:5600
num(:,2)=num(:,1)-663500000;
...
wz_estq = wz_est(2:4,n+2:m)./pi*180*3600;   %  °/s        qz_est(1,n+2)
wdzq = wdz(2:4,n+2:m)./pi*180*3600;         %  度/s 
figureWdata2(wz_estq,wdzq,n+2,length(qz_est),h) %滤波的 原始的
[psd_West,psd_West_f,psd_Worig,psd_Worig_f]= psdcom (wz_estq,wdzq,h);
figureWpsd2(psd_West,psd_Worig,psd_West_f) 
...
figureqdata2(att_estd,att_origd,n+2,length(qz_est),h) %滤波的 原始的
%以 semilogy 形式绘制功率谱
